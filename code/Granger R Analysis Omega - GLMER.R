...
# source("/Users/au704315/Documents/R/geom_flat_violin.R")
library(huxtable)
library(rstatix)
library(Rmisc)
library(broom.mixed)
...
# Read in csv data ---------
# setwd("/Users/au704315/Documents/Work-MARCS/EXPERIMENTS/Omega Ensemble/Omega Ensemble Rehearsal/Analysis_2020/Sanket_analysis/Granger Causality")

data_brahms <- data.frame(read_csv('C:/San/PhD/Project/Causality/GrangerCausality/PetersCode/GrangerCausality-Peter/ICMPCanalysis2/Results/R Analysis/Final_Brahms_Results_GC.csv'))
data_borodin <- data.frame(read_csv('C:/San/PhD/Project/Causality/GrangerCausality/PetersCode/GrangerCausality-Peter/ICMPCanalysis2/Results/R Analysis/Final_Borodin_Results_GC.csv'))

## Optional scaling
# data_brahms[c('F_M1_M2', 'F_M2_M1', 'avgF')] <- lapply(data_brahms[c('F_M1_M2', 'F_M2_M1', 'avgF')], function(x) c(scale(x))) # center F value data
# data_borodin[c('F_M1_M2', 'F_M2_M1', 'avgF')] <- lapply(data_borodin[c('F_M1_M2', 'F_M2_M1', 'avgF')], function(x) c(scale(x))) # center F value data
...
# Convert variables to factors (not necessary)
...
# summary_data_p_prop <- summarySEwithin(data = data_p_prop, measurevar = "p_sig", withinvars = "texture", idvar = NULL,
#                                        na.rm = TRUE, conf.interval = 0.95, .drop = TRUE)
...
## For computing confidence intervals (computationally slow)
# Pv_long_melody_ ci <- confint(Pv_long_melody) # Compute confidence intervals
# Pv_long_melody_ci
